...
long long 		total_requestGPT_time = 0;
...
int main(int argc, char** argv){
	...
	//filename = (char *)"homes16.blkparse";
	//filename = (char *)"hm_0.csv";
	filename = (char *)"ssdtrace.txt";
	//std::string splitter;

	std::ifstream file(filename);
	//if (strcmp(filename, "homes16.blkparse") == 0) {
		std::string splitter = " ";
	//}else{
		//std::string splitter = ",";
	//}
	...
	if (file.is_open()) {
		while (std::getline(file, str))
		{
			...
			// 128166477394345573,hm,1,Read,383496192,32768,113736-----temp 1-7
			...
			if (operation == temp1 || operation2 == temp1) { //write operation
				...
				if (TotalWriteNumber % gpt_period == 0){
					...
					auto duration = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count(); // 单位为微秒
					total_requestGPT_time += duration;
					//window_size = 3000;
	...
	printf("Total requestGPT execution time: %lld microseconds\n", total_requestGPT_time);
