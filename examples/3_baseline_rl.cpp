...
static struct state get_next_state(struct state* s, int32_t _current_action, int w, int SLC_write) {
	struct state ns;
...
int main(int argc, char** argv){
	...
	if (file.is_open()) {
		while (std::getline(file, str))
		{
			...
			if (newwrite_k > write_period_k) {
				hot_threshold = slice_clusting();
				slice_clusting_kmeans();
				period_count_k = period_count_k + 1;
				newwrite_k = 0;
			}
			...
			if (newwrite > write_period) {
				...
				//******** starting with RL ********//
				next_state = get_next_state(&current_state, current_action, newwrite, SLC_write);
